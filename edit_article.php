...
// Lấy thông tin bài viết từ cơ sở dữ liệu để sửa
...
// Cập nhật bài viết sau khi người dùng nhấn nút "Sửa"
if (isset($_POST['Sửa'])) {
    ...
    if (empty($tieude) || empty($ten_bhat) || empty($ma_tloai) || empty($tomtat) || empty($ma_tgia) || empty($ngayviet)) {
        ...
        $sql = "UPDATE baiviet 
                SET tieude = :tieude, ten_bhat = :ten_bhat, ma_tloai = :ma_tloai, tomtat = :tomtat, ma_tgia = :ma_tgia, ngayviet = :ngayviet 
            ...
            <div class="col-sm">
                ...
                <form action="edit_article.php?ma_bviet=<?php echo $ma_bviet; ?>" method="post">
                    <div class="input-group mt-3 mb-3">
                        ...
                        <input type="text" class="form-control" name="txtMaBViet"
                            value="<?php echo $article['ma_bviet']; ?>" readonly>
                    </div>

                    <div class="input-group mt-3 mb-3">
                        <span class="input-group-text">Tiêu đề</span>
                        <input type="text" class="form-control" name="txtTieuDe"
                            value="<?php echo $article['tieude']; ?>" required>
                    </div>

                    <div class="input-group mt-3 mb-3">
                        <span class="input-group-text">Tên bài hát</span>
                        <input type="text" class="form-control" name="txtTenBhat"
                            value="<?php echo $article['ten_bhat']; ?>" required>
                    </div>

                    <div class="input-group mt-3 mb-3">
                        <span class="input-group-text">Mã thể loại</span>
                        <input type="text" class="form-control" name="txtMaTLoai"
                            value="<?php echo $article['ma_tloai']; ?>" required>
                    </div>

                    <div class="input-group mt-3 mb-3">
                        <span class="input-group-text">Tóm tắt</span>
                        <input type="text" class="form-control" name="txtTomTat"
                            value="<?php echo $article['tomtat']; ?>" required>
                    </div>

                    <div class="input-group mt-3 mb-3">
                        <span class="input-group-text">Mã tác giả</span>
                        <input type="text" class="form-control" name="txtMaTGia"
                            value="<?php echo $article['ma_tgia']; ?>" required>
                    </div>

                    <div class="input-group mt-3 mb-3">
                        <span class="input-group-text">Ngày viết</span>
                        <input type="date" class="form-control" name="ngayviet"
                            value="<?php echo $article['ngayviet']; ?>" required>
                    </div>
